...
const handlerFactory = require('./kana-error-handler');
...
describe('middleware#grammar-error-handler', function () {
    ...
    it('should call next if error is not KanaError', function () {
        let next = sinon.spy();
    ...
    it('should respond with status code ' + http.BAD_REQUEST + ' (bad request)', function () {
        let err = new KanaError('grammar message');
